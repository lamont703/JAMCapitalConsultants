...
# JAM Capital Backend - Create Web App with B1 Tier in East US 2
# This script creates the Web App using B1 tier in East US 2 where quota is available

set -e  # Exit on any error

# Configuration variables - Using East US 2 for Basic quota availability
RESOURCE_GROUP="JAM_resource_group"
LOCATION="East US 2"  # Changed to East US 2 where quota is available
...
# Create App Service Plan with B1 tier in East US 2
create_app_service_plan() {
    log_info "Creating App Service Plan with B1 tier in $LOCATION: $APP_SERVICE_PLAN"
    
    if az appservice plan show --name "$APP_SERVICE_PLAN" --resource-group "$RESOURCE_GROUP" &> /dev/null; then
        log_warning "App Service Plan $APP_SERVICE_PLAN already exists"
        
        # Check current tier and location
        CURRENT_SKU=$(az appservice plan show --name "$APP_SERVICE_PLAN" --resource-group "$RESOURCE_GROUP" --query "sku.name" -o tsv)
        CURRENT_LOCATION=$(az appservice plan show --name "$APP_SERVICE_PLAN" --resource-group "$RESOURCE_GROUP" --query "location" -o tsv)
        log_info "Current SKU: $CURRENT_SKU, Location: $CURRENT_LOCATION"
        
        if [ "$CURRENT_SKU" != "B1" ]; then
            log_warning "Updating plan to B1 Basic tier..."
        ...
        log_info "Creating new B1 Basic App Service Plan in East US 2..."
        ...
        log_success "B1 Basic App Service Plan created successfully in East US 2"
    fi
}
...
display_success_info() {
    ...
    echo ""
    echo "💡 **B1 Basic Tier Benefits**:"
    echo "   • Unlimited compute time"
    echo "   • Custom domains & SSL"
    echo "   • Better performance than Free tier"
    echo "   • Professional hosting"
    echo ""
    echo "🌍 **Location Notes**:"
    echo "   • East US 2 is very close to East US"
    echo "   • Your Cosmos DB (East US) will work perfectly"
    echo "   • Minimal latency difference"
    echo ""
}
...
main() {
    echo ""
    echo "🚀 Creating JAM Capital Web App with B1 Tier in East US 2"
    echo "========================================================="
    echo ""
    echo "📍 Using East US 2 because Basic quota is available there"
    echo "🔗 Your existing resources (Cosmos DB, Storage) will work perfectly"
    echo ""
